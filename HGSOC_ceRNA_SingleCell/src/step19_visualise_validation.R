...
fdr_sig <- readRDS(args[2])

visualise_validation <- function(fdr_sig, dir_out) {
    print("Function visualise_validation")

    set.seed(981214)

    edges_long <- fdr_sig %>%
        select(lncRNA, miRNA, pcg, module) %>%
    ...
    pdf_path <- file.path(dir_out, "FDR_sig_Network.pdf")
    cairo_pdf(pdf_path, width = 15, height = 15)
    print(plot1)
    dev.off()
}

visualise_validation(fdr_sig, dir_out)